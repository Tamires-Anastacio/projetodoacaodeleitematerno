...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validações simples
    if (empty($nome) || empty($cnpj) || empty($cidade) || empty($especialidade) || empty($telefone) || empty($email) || empty($uf) || empty($num) || empty($senha_hash) || empty($rua)) {
        die("Por favor, preencha todos os campos obrigatórios!");
    }

    // Hash da senha (recomendado para segurança)
    $senha_hash = password_hash($senha_hash, PASSWORD_BCRYPT); // Hashing da senha para segurança

    try {
        $sql = "INSERT INTO instituicao (nome, cnpj, certificacao, cidade, num, uf, rua, senha_hash, telefone, email, especialidade )
